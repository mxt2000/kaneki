...
module.exports.handleEvent = async ({ Users, api, event}) => {
  ...
  const unbanAfter = 3000000000; // 600000 millisecond (10 phút) 
	...
	if ((global.client.autoban[senderID].timeStart + thoi_gian_spam) <= Date.now()) {
	  ...
	  if (global.client.autoban[senderID].number >= so_lan_spam) {
	    const time = moment.tz("Asia/Ho_Chi_minh").format("DD/MM/YYYY HH:mm:ss");
			...
			data.banned = true;
			data.reason = `spam bot ${so_lan_spam} lần/${thoi_gian_spam/100000}phút`;
			data.autoban = {
			  timeStart: Date.now(),
			  unbanAfter
			};
  		...
  		api.sendMessage({
  		  body: senderID + " | " + dataUser.name + `\n🤍 Bạn đã bị cấm sử dụng bot=)) ${unbanAfter/3000000000}phút với lý do: spam bot \n🖤 Auto unban sau ${Math.floor(unbanAfter/3000000000)}phút`,

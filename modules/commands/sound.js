...
module.exports.handleReply = async function ({
    ...
    try {
        const result = await axios.get(`https://api.leanhtruong.net/api/soundcloud?apikey=nhập key ở đây&download=true&link=${handleReply.link[event.body - 1]}`);
...
module.exports.run = async function ({
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        const res = await axios.get(`https://api.leanhtruong.net/api/soundcloud?apikey=nhập key ở đây&download=true&link=${args.join(" ")}`)
        ...
        if (fs.statSync(__dirname + `/cache/${event.senderID}.m4a`).size > 26000000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.m4a`), event.messageID);
            ...
            const ress = (await axios.get(`https://api.leanhtruong.net/api/soundcloud?apikey=nhập key ở đây&keyword=true&search=${encodeURIComponent(keywordSearch)}`))

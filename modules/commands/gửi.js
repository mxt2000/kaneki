...
module.exports.run = async ({ args, api, event }) => {
	...
	if (!permission.includes(event.senderID)) return api.sendMessage("_callad Có Thằng Định Phá Code Này Ông Chủ:))", event.threadID, event.messageID); fs = global.nodemodule["fs-extra"];
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';

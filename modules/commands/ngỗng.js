...
module.exports.run = async ({ api, event }) => {
 ...
 let callback = function () {
     ...
     }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/ngong.${ext}`), event.messageID);
    };
    request(res.data.url).pipe(fs.createWriteStream(__dirname + `/cache/ngong.${ext}`)).on("close", callback);
   })
}